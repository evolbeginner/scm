...
library(phytools)
library(phangorn)
library(TreeTools, warn.conflicts=F)
library(getopt)
library(this.path)
library(base)
...
generatePieJoint <- function(t){
	a <- numeric(length(traits))
	nodes <- sapply(node2tips, FUN=function(tips){findMRCA(t, tips)})
	node2traits <- sapply( nodes, FUN=function(x){t$node.label[x-t$Nnode-1]} )
	paste(node2traits, collapse='|')
}
...
drop_tip <- function(t, dropped_tips){
	drop.tip(t, dropped_tips)
}
...
outfile <- NULL
itol_outfile <- NULL
tree_w_node_label_outfile <- 'tree_w_node_label.pdf'
...
command=matrix(c( 
	...
	'cex', 'c', 2, 'character',
	'itol', 'i', 2, 'character',
	'marginal', 'marginal', 0, 'logical',
	'joint', 'j', 0, 'logical',
	'species_list', 's', 2, 'character',
...
if(! is.null(args$joint)){
	is_marginal <- F
}
if(! is.null(args$joint)){
	is_joint <- T
}
if(! is.null(args$species_list)){
...
if(is.null(ml_tree)){
	...
	mapped_tree <- ml_tree
	dropped_tips <- setdiff(mapped_tree$tip.label, tree1$tip.label)
	mapped_tree <- drop.tip(mapped_tree, dropped_tips) 
}

if(!is.null(species_list)){
	include_species <- unlist(read.table(species_list))
	dropped_tips <- setdiff(mapped_tree$tip.label, include_species)
	tree <- as.multiPhylo(lapply(tree, drop_tip, dropped_tips))
	mapped_tree <- drop.tip(mapped_tree, dropped_tips) 
}
...
################################################
if(is_joint){
	pie.list.joint <- lapply(c(tree), generatePieJoint)
	cat("#node", fill=T)
	cat(node_to_two_taxon_name, collapes="\n", fill=T)
	cat("#joint_prob")
	ll <- unlist(table(unlist(pie.list.joint)))
	print(ll/sum(ll))

	pdf(tree_w_node_label_outfile)
	plotTree(mapped_tree, pic_type)
	nodelabels()
	dev.off()
	q()
}
